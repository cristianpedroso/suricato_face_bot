    ...
    if (messageText) {
        switch (users[senderID]) {
            ...
            case ALERT_ADD:
                ...
                sendTextMessage(senderID,"Agora precisamos de seu endereço mas fique atento.",ALERT_ADD,function () {
                    sendTextMessage(senderID,"É necessário conter: Rua, Cidade, Estado nesse formato");
                } );
                users[senderID] = ALERT_NAME;
                break;
            case ALERT_NAME:
                usersAlert[senderID] = {
                    name: usersAlert[senderID].name,
                    address: messageText
                };
                console.log(usersAlert);
                sendTextMessage(senderID,"Pronto agora está tudo certo!",ALERT_NAME,function () {
                    //TODO ADD IMATE
                    sendTextMessage(senderID,"Qualquer coisa que nós suricatos obervarmos de diferente vamos alertar você!");
                } );
                users[senderID] = START;
                break;
            case WANT_PHONE:
        ...
        message: {
            attachment: {
                ...
                payload: {
                    ...
                    text: "Primeiro passo, precisamos saber o que você deseja?",
                    buttons: [
                        {
                            type: "postback",
...
function receivedPostback(event) {
    ...
    switch (payload) {
        case 'start_thread':
            ...
            break;
        case 'return_help':
            sendTextMessage(senderID, "Tudo bem, o importante agora é manter a calma.",HELP_PHONE, function (){
                sendTextMessage(senderID, "Precisamos que você diga seu numero de telefone no formato (xx) xxxxx-xxxx.");
            });
            users[senderID] = HELP_PHONE;
            break;
        case 'return_alert':
            sendTextMessage(senderID, "Vamos precisar de algumas informações para conseguir avisar se você está ou não em uma area de risco.",ALERT_ADD, function (){
                sendTextMessage(senderID, "A primeira informação que precisamos é seu nome completo!");
            });
 ...
 *
 */
function sendTextMessage(recipientId, messageText, metadata, callback) {
